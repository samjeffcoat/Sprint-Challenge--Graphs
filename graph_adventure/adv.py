...
def find_move(previous=None):
    ...
    for i in visited:
        if i == curRoomID:
            ...
            return
    visited[curRoomID] = {}
    print(f"Adding room {curRoomID} to visited")
...
"""def bfs(self, starting_direction, ending_direction):
    qq= Queue()
    visited = set()
    qq.enqueue([starting_direction])
    while qq.size()> 0:
        path == qq.dequeue()
        vertex = path[-1]
        if vertex not in visited:
            if vertex == ending_direction:
                return path
            visited.add(vertex)"""
...
if len(visited_rooms) == len(newyorkcity):
    print(
        f"TESTS PASSED: {len(traversalPath)} moves, {len(visited_rooms)} rooms visited")
else:
    print("TESTS FAILED: INCOMPLETE TRAVERSAL")
    print(f"{len(newyorkcity) - len(visited_rooms)} unvisited rooms")
